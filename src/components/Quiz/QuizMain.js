...
console.log(pus)
  ...
  useEffect(() => {
    fetch("https://cors-anywhere.herokuapp.com/https://portal-employee-service.herokuapp.com/interview")
      .then(res => res.json())
      .then(
        (result) => {
          setIsLoaded(true);
...
console.log(reverse)
  ...
  return (
    <div>
      ...
      {score === 3 ? <CorrectStatusBar /> : <StatusBar numberQuestions={numberQuestions} count={count} timer={timer} />}

      <QuestionTitle questions={questions} />
      <Answer answers={answer} selectAnswer={selectAnswer} checked={checked} reverse={reverse}/>
      <NextQuestion nextQuestion={nextQuestion} truAnswer={truAnswer} BackToQuestion checked={checked} backToQuestion={backToQuestion} timer={timer} Close={Close} />
